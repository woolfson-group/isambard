...
class PreferredMmolTestCase(unittest.TestCase):
    ...
    def test_preferred_mmol_1cwa(self):
        ...
        code = '1cwa'
        self.assertEqual(isambard.add_ons.filesystem.preferred_mmol(code=code), 1)

    def test_preferred_mmol_4nfw(self):
        """ Preferred mmol number for the PDB code 1cwa should be 4 """
        code = '4nfw'
        self.assertEqual(isambard.add_ons.filesystem.preferred_mmol(code=code), 4)

    def test_nonsense_raises_value_error(self):
        code = "nonsense_code"
        self.assertRaises(OSError, isambard.add_ons.filesystem.preferred_mmol, code)
...
class GetMmolTestCase(unittest.TestCase):
    ...
    def test_get_mmol_file_length_4nfw_1(self):
        ...
        code = '4nfw'
        self.assertEqual(len(isambard.add_ons.filesystem.get_mmol(code=code, mmol_number=1)), 110328)

    def test_nonsense_is_none(self):
        """A non-existent PDB code should return None. """
        code = "nonsense_code"
        self.assertIsNone(isambard.add_ons.filesystem.get_mmol(code=code))
...
class PdbeStatusTestCase(unittest.TestCase):
    ...
    def test_status_code_valid(self):
        ...
        code = '1cwa'
        self.assertEqual(isambard.add_ons.filesystem.pdbe_status_code(code), 200)

    def test_status_code_invalid(self):
        code = 'invalid_code'
        self.assertEqual(isambard.add_ons.filesystem.pdbe_status_code(code), 404)
