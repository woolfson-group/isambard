...
from tools.file_parsing import get_protein_dict_from_parmed, olderado_best_model
...
class FileSystem:
    ...
    def protein_info(self):
        return get_protein_dict_from_parmed(cif_file=self.mmcif)

    @property
    def ampals(self):
        """ Dict of ampal Assembly objects corresponding to the mmols associated with code.

        Notes
        -----
        Runs self.mmols and self.protein_info (and therefore self.mmcif) - so mmol and mmcif files will be written.
        The .cif file corresponding to code is parsed and details are appended to the Assembly via the attribute
        'structural_information'.
        The key 'mmol_number' is added to the Assembly tags.
        ...
        amapals : dict
            Keys : int
                mmol number
            Values : Assembly
                Annotated ampal Assembly object.

        """
        ampals = {}
        for i, mmol_file in self.mmols.items():
            if i == self.preferred_mmol:
                preferred = True
            else:
                preferred = False
            mmol_tags = {'mmol_number': i, 'preferred_mmol': preferred}
            a = convert_pdb_to_ampal(pdb=mmol_file, path=True, pdb_id=self.code)
            if type(a) == AmpalContainer:
                for x in a:
                    x.tags.update(mmol_tags)
                    x.structural_information = self.protein_info
            else:
                a.tags.update(mmol_tags)
            a.structural_information = self.protein_info
            ampals[i] = a
        return ampals
...
def fetch_all_assemblies(pdb_code, data_dir=None):
    """ Retrieves the all structure for a PDB entry and returns as an Ampal Container of Assemblies.
    ...
    pdb_code : str
        4-letter PDB ID
    data_dir : str
        Path to directory where PDB files are stored, if not as defined in global settings.

    Returns
    -------
    ampal_container : [AmpalContainer]
        Protein structure as AMPAL object
    """
    pdb = pdb_code.lower()
    try:
        filesystem = FileSystem(pdb, data_dir=data_dir)
    except ValueError as v:
        print("Cannot get any mmol files for {0} - {1}.".format(pdb, v))
        return None
    except OSError as o:
        print("Skipping", pdb, '-', o)
        return None
    ampals = filesystem.ampals
    protein_info = filesystem.protein_info
    if 'NMR' in protein_info['experimental_method']:
        ampal = list(ampals.values())[0]
        if isinstance(ampal, AmpalContainer):
            ampal_container = ampal
        else:
            ampal_container = AmpalContainer([ampal], id=pdb)
    else:
        ampal_container = AmpalContainer([x for x in ampals.values()], id=pdb)
    ampal_container.structural_information = protein_info
    return ampal_container


def fetch_assembly(pdb_code, mmol_or_state='preferred', data_dir=None):
    """ Retrieves the structure for a PDB entry and returns as an Assembly
    ...
    pdb_code : str
        4-letter PDB ID
    mmol_or_state : str
        Number of mmol to convert to assembly
    data_dir : str
    ...
    -------
    assembly : [Assembly]
        Protein structure as AMPAL assembly
    """
    pdb = pdb_code.lower()
    ampal_container = fetch_all_assemblies(pdb, data_dir=data_dir)
    if not ampal_container:
        print("Could not find an assembly for {0}.".format(pdb))
        return None
    protein_info = ampal_container.structural_information
    if mmol_or_state == 'preferred':
        if 'NMR' in protein_info['experimental_method']:
            if len(ampal_container) == 1:
                assembly = ampal_container[0]
            else:
                best_conformation = olderado_best_model(pdb)
                if best_conformation:
                    assembly = ampal_container[str(best_conformation)]
                else:
                    print("No preferred assembly for {0} - using first state.".format(pdb))
                    assembly = ampal_container[0]
        else:
            if len(ampal_container) == 1:
                assembly = ampal_container[0]
            else:
                preferred_assemblies = [x for x in ampal_container if x.tags['preferred_mmol']]
                if len(preferred_assemblies) == 1:
                    assembly = preferred_assemblies[0]
                else:
                    print("No preferred assembly for {0} - using first mmol.".format(pdb))
                    assembly = ampal_container[0]
    else:
        if 'NMR' in protein_info['experimental_method']:
            state_numbers = {x.id.split('_')[-1]: x for x in ampal_container}
            try:
                assembly = state_numbers[str(mmol_or_state)]
            except Exception as e:
                print("There is no mmol corresponding to {0}.".format(e))
                raise
        else:
            mmol_numbers = {x.tags['mmol_number']: x for x in ampal_container}
            try:
                assembly = mmol_numbers[int(mmol_or_state)]
            except Exception as e:
                print("There is no mmol corresponding to {0}.".format(e))
                raise
    if isinstance(assembly, Polypeptide):
        assembly = Assembly(molecules=[assembly], assembly_id=ampal_container.id)
    return assembly
